...
# issue-labels-by-name Specification

## Purpose

Let a caller name a label instead of looking up its numeric id, on every tool that
assigns one: `labels` when creating an issue, `add_issue_labels`, `remove_issue_labels`,
and `set_labels` on `update_issue`. Names are resolved to ids inside the server and an
unresolvable one aborts the call, because Forgejo's own name handling drops the names it
does not recognise and still answers 200 — a half-applied request that reads as success.
This capability also carries the `assignees` and `milestone` arguments on `create_issue`,
so one call produces a fully formed issue.
