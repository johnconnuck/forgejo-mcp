...
### Session completion: two notes on the block above

The Session Completion steps above are generated by `bd` and get rewritten
whenever the integration block is regenerated, so these notes live outside it
and take precedence:

- **`bd dolt push` works — run it.** An earlier note here said to skip it
  because `.beads/config.yaml` pointed `sync.remote` at the pre-migration
  Codeberg URL. That is no longer true; it points at
  `https://hub.dolt.b4mad.industries/forgejo_mcp`, and the push succeeds
  (verified 2026-09-16). Issue data still never enters git, so beads are
  stranded locally if you skip it.
